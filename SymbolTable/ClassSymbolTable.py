...
class ClassSymbolTable(SymbolTable):
    ...
    def presentInInheritedScopeChain(self,name):
        return (not self.isGlobalScope) and ((name in self.symbols) or (self.parent and (not self.parent.isGlobalScope) and self.parent.presentInInheritedScopeChain(name)))

    def functionPresentInInheritedScopeChain(self,name):
        return (not self.isGlobalScope) and ((name in self.functions) or (self.parent and (not self.parent.isGlobalScope) and self.parent.functionPresentInInheritedScopeChain(name)))
    ...
    def getFunctionDefinitionFromInstanceOrInheritedOnly(self, name):
        if not self.isGlobalScope:
            ...
            elif self.parent and (not self.parent.isGlobalScope):
    ...
    def getFromInstanceOrInheritedOnly(self,name):
        ...
        if (not self.isGlobalScope):
            if self.present(name):
                ...
                return self.parent.getFromInstanceOrInheritedOnly(name)
            
        raise Exception(f"Variable '{name}' not defined")
        
    def setInInstance(self,name,value,type,parentSymbolTable=None,isInstance=True):
        # if self.functionPresentInInheritedScopeChain(name):
